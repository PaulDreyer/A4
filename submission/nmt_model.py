...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (last_h, last_c) = self.encoder(pack_X)
        enc_hiddens, len_hiddens = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)        
        init_decoder_hidden = self.h_projection(torch.cat((last_h[0], last_h[1]), dim=1))
        init_decoder_cell = self.c_projection(torch.cat((last_c[0], last_c[1]), dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for yt in torch.split(Y, 1):
            yt_sq = torch.squeeze(yt, dim=0)
            yt_bar = torch.cat((yt_sq, o_prev), dim=1)
            dec_state, o_t, e_t = self.step(yt_bar, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        tanhV_t = torch.tanh(V_t)
        O_t = self.dropout(tanhV_t)
        ### END CODE HERE
